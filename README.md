...
- **Calendar creation wizard** — a guided 5-step flow (`/calendars/wizard`, the single entry point for creating a calendar from the dashboard and side menu): name → cover (reuses the shared crop component) → family members → events → review. The member hub lets you add, edit, or skip members up front (optional drag-and-drop profile photo with preview/replace/remove, name, birth date, optional anniversary date, and the four tag lists), with client-side validation and an XHR submit that reports upload progress. The events step previews the birthday/anniversary events that are auto-generated from the added members (badged as auto-generated) and lets you add skippable manual events with the same fields as the events form. The review step is deliberately calm and minimal: a small color-palette swatch row (rounded swatches for the "ברירת מחדל" option plus each `config/themes.php` preset, subtle ring on the active one, live caption naming the selection) sits above a live preview of the current month (same grid styling as the month view, LTR grid inside the RTL layout, welcome-page responsive scroll pattern) populated with the session's members/events and holidays. Selecting a swatch applies the palette to the preview immediately (no apply step) and the choice persists across all 12 month pages on submit. Members are scoped to the new calendar and their photos land in the calendar's auto-synced media folder; a member photo also becomes the background of their birth-month page automatically. Creating a calendar from the classic form still works and shares the same `CalendarCreationService`. On mobile the member/event form actions and the footer navigation stack full-width (side-by-side from `sm:`) so the long Hebrew button labels never overflow the card
...
- **Design settings per month** — font, overlay opacity, day-box background/text colors and opacity, weekday color, background image, and an adjacent-month days toggle, in a collapsible accordion. Each month's background resolves manually-set first (library pick or direct upload), then any auto-set member photo
...
- **Calendar creation wizard** — `GET/POST /calendars/wizard` is served by `CalendarWizardController` (`calendars.wizard` / `calendars.wizard.store`, registered before the `calendars` resource so the binding doesn't swallow it). `create()` renders the `calendarWizard` Alpine component (5 steps: name, cover, members, events, review) in `resources/views/calendars/wizard.blade.php`, passing the current month's grid data (holidays via `IsraeliHolidaysService`, Hebrew dates, default styles via `MonthPageStyleService::resolve` on a defaults-based `MonthPage`) so the review step can render a live preview. `store()` runs `CalendarCreationService::create` plus the member/photo loop and manual-event loop inside a single `DB::transaction`, applies the chosen `config/themes.php` preset to all 12 month pages when one is selected, and returns a JSON redirect so the view can submit via XHR with a progress bar. Manual events link to wizard members via temporary client keys (resolved to member ids after they are created), while birthday/anniversary events are generated by the `FamilyMemberObserver` — never posted from the client. The review preview injects the session's members/events client-side (recurring dates resolved against the preview year, mirroring `FamilyEventGeneratorService::resolveForYear`) and live-applies theme fields to the grid. Validation lives in `StoreCalendarWizardRequest`, which throws a 422 JSON response (the app's global exception handler only JSON-formats `api/*` errors). Calendar + 12 month pages are created by `CalendarCreationService`, shared with the classic `CalendarController::store`. Each member's form leads with an optional profile-photo dropzone (click or drag-drop, preview, replace, remove) that stages a single `File` client-side until the review submit; member photos are attached via `FamilyMemberMediaService::storeImages`, the same service the member CRUD uses, so the photo lands in the member's calendar-scoped media folder and automatically becomes the background of their birth-month page.
...
- **Media library** — the `User` model uses `InteractsWithMedia` with a `user_media` collection and a `thumb` conversion. A `MediaPolicy` scopes every item to its owner (view/update/delete are forbidden across users), and deleting an item first nulls any `month_pages.background_media_id` references and clears any auto-set backgrounds so no orphaned backgrounds remain. A month background can come from the media library (`background_media_id`, resolved first), a direct upload (`custom_image_path`), an auto-set member photo (`auto_background_media_id`, resolved next), or the legacy path (`background_image_path`), with a picker in the month design settings.

- **Auto birth-month backgrounds** — when a family member's photo is added (wizard or member form), `FamilyMemberMediaService::storeImages` hands it to `MemberBirthMonthBackgroundService`, which sets it as the `month_pages` auto background for the month of the member's birthday. Conflict rules: a month already carrying a manual background is never touched, and a month already claimed by another member keeps the first photo; a new photo of the same member replaces their own auto-set background. The member "owns" their auto background: removing the photo or deleting the member clears it, and changing the birth date moves it to the new month (only when that month is free). Choosing/uploading a manual background in the month design settings clears the auto one.
...
└── Services/
    ...
    ├── MemberBirthMonthBackgroundService
